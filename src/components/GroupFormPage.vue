...
const handleSubmit = async () => {
  ...
  await formRef.value.validate((valid) => {
    if (valid) {
      ...
      emit('submit', isEdit.value && props.group
        ? { listId: targetListId, groupId: props.group.id, name: form.value.name, color: form.value.color }
        : { listId: targetListId, name: form.value.name, color: form.value.color })
    }
  })
}
</script>
